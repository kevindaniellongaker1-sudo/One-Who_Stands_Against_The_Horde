...
- Wildlife is a living ecosystem (`WildlifeTurn` in CombatEnemyAI.cs, 3 actions/beast). Ecosystem state on `Enemy`: `BeastTarget`, `ProvokedBy` (last creature that injured it), `ProvokedByPlayer` (set by all player damage in CombatAttacks.cs), `CorpseMeals`. `BeastCritical` = ≤5% HP or ≤2. Wolves hunt deer/boar with close→bite→circle-away; devour kills (+2-3 HP per quarter, 4/kill); critical wolves limp toward hornless deer. Hornless deer bolt from wolves/bears/attackers and graze (+2) when safe; antlered deer gore their tormentor. Boars charge ANYTHING except boars/deer — players, horde, bears — and keep their target. Bears press their target every action; critical bears flee to cave (sleep +1-2), river (fish 75%, +2-4) or tree. Non-beast NPCs bitten by a beast turn and fight it (`ProvokedBy` retaliation in EnemyTurn). `TryBeastStep` handles beast movement: enemies colliding knock down whichever has lower max(Str,Dex); river squares set `Wading` (next step lost). Kills auto-skin: hides 1d9-1, meat 2d6-2.
- Rivers (`Rivers` set, ~70% of battlefields): winding water down the left-center. Crossable at DOUBLE movement (players pay 2 in StepMovement; enemies lose their next step via `Wading`). Drawn blue in the window, `~` on the console map.
- Carriers: arrows fill `QuiverCap` (archers start with 50 free), potions `PotionPouchCap` (2 free for everyone), throwing weapons `ThrowBandCap`; overflow spills into the pack via `PackLoad` (arrows bundle 10/slot). `HasBagOfHolding` = truly unlimited. `HasWorkshopHammer` gates Artisan crafting (Artisans start with one).
